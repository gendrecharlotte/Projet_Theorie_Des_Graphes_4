...
void Interface::affichage_interface(DifferentsAeroports aeroports,int type)
{
    ...
    perturbation=load_bitmap("images/perturbation.bmp",NULL);
    ///DEFINITION POLICE
    ...
    while(!key[KEY_ESC])
    {
        ...
        ///PERTURBATION
        draw_sprite(page,perturbation,441,586);
...
void Interface::test(AllAvions avions,DifferentsAeroports aeroports,Simulation simulation, Voyageur* voyageur, int choix)
{
    ...
    std::vector<int> vitesses_bases;
    for(auto a:avions.get_liste_avions())
    {
        vitesses_bases.push_back(a->get_vitesse());
    }
    show_mouse(screen);

    while(!key[KEY_ESC]&&total_UT<=50)
    {
        ///NETTOYAGE BUFFER
        clear_bitmap(page);
        ///AFFICHAGE BITMAP PAR DESSUS BUFFER
        ...
        for(auto a:avions.get_liste_avions())
        {
            ...
            if((mouse_x>=a->get_coo().get_x() && mouse_x<=a->get_coo().get_x()+25) && (mouse_y>=a->get_coo().get_y() && mouse_y<=a->get_coo().get_y()+25))
            {
                ...
                textprintf_ex(page, cn,a->get_coo().get_x()+25, a->get_coo().get_y()+57,red,white,"%d",a->get_vitesse());
            }
            if(a->get_coo().get_x()>=441 && a->get_coo().get_x()<=759 && a->get_coo().get_y()>=586 && a->get_coo().get_y()<=672)
            {
                std::cout<<a->get_matricule()<<" est en zone de turbulence a UT "<<total_UT<<" !"<<std::endl;
                a->set_vitesse(20);
            }
            else
            {
                a->set_vitesse(vitesses_bases[i]);
            }
            ...
            if(traj.size()!=0 && a->get_etat()>=2)
            {
                ...
                {
                    coo.set_y(coo.get_y()+a->get_vitesse());
                    a->set_coo(coo);
                }
                ...
                {
                    coo.set_x(coo.get_x()+a->get_vitesse());
                    a->set_coo(coo);
                }
                ...
                if(abs(a->get_coo().get_y()-traj.top()->get_Localisation().get_y())<=a->get_vitesse() && abs(a->get_coo().get_x()-a->get_trajet().top()->get_Localisation().get_x())<=a->get_vitesse())
                {
            ...
            for(auto k:avions.get_liste_avions())
            {
                ...
                if (a->get_coo().get_x()<test_x_max && a->get_coo().get_x()>test_x_min && a->get_coo().get_y()<test_y_max && a->get_coo().get_y()>test_y_min && a->get_coo().get_z()==k->get_coo().get_z())
                {
                    if(a!=k)
                    {
                        ...
                        if(a->get_coo().get_z()>=10 && a->get_coo().get_z()<20 && a->get_type()==1 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_11.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=10 && a->get_coo().get_z()<20 && a->get_type()==2 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_22.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=10 && a->get_coo().get_z()<20 && a->get_type()==3 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_33.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=20 && a->get_coo().get_z()<30 && a->get_type()==1 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_111.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=20 && a->get_coo().get_z()<30 && a->get_type()==2 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_222.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=20 && a->get_coo().get_z()<30 && a->get_type()==3 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_333.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=30 && a->get_type()==3 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/scratch.bmp",NULL));
                        }
                        ///--------------------------------------------Si il s'agit de l'avion que nous avons selectionné -----------------------------------------///
                        else if(a->get_coo().get_z()>=10 && a->get_coo().get_z()<20 && a->get_type()==1 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_11_selec.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=10 && a->get_coo().get_z()<20 && a->get_type()==2 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_22_selec.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=10 && a->get_coo().get_z()<20 && a->get_type()==3 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_33_selec.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=20 && a->get_coo().get_z()<30 && a->get_type()==1 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_111_selec.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=20 && a->get_coo().get_z()<30 && a->get_type()==2 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_222_selec.bmp",NULL));
                        }
                        else if(a->get_coo().get_z()>=20 && a->get_coo().get_z()<30 && a->get_type()==3 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
                        {
                            a->set_img(load_bitmap("images/avion_333_selec.bmp",NULL));
                        }
            ...
            if(a->get_coo().get_z()==0 &&a->get_type()==1 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
            {
                a->set_img(load_bitmap("images/avion_1.bmp",NULL));
            }
            else if(a->get_coo().get_z()==0 && a->get_type()==2 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
            {
                a->set_img(load_bitmap("images/avion_2.bmp",NULL));
            }
            else if(a->get_coo().get_z()==0 && a->get_type()==3 && (voyageur->get_avion()).get_matricule()!=a->get_matricule())
            {
                a->set_img(load_bitmap("images/avion_3.bmp",NULL));
            }
            ///--------------------------------------Si il s'agit de l'avion selectione ----------------------------------------///
            else if(a->get_coo().get_z()==0 &&a->get_type()==1 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
            {
                a->set_img(load_bitmap("images/avion_1_selec.bmp",NULL));
            }
            else if(a->get_coo().get_z()==0 && a->get_type()==2 && (voyageur->get_avion()).get_matricule()==a->get_matricule())
            ...
            //affichage des infos à la souris
            if((mouse_x>=a->get_coo().get_x() && mouse_x<=a->get_coo().get_x()+25) && (mouse_y>=a->get_coo().get_y() && mouse_y<=a->get_coo().get_y()+25))
            {
                //std::cout<<a->get_matricule()<<std::endl;
                textprintf_ex(page, cn,a->get_coo().get_x()+25, a->get_coo().get_y()+25,red,white,"%d",a->get_matricule());
