...
AllAvions::AllAvions(std::string nomFichier) //le constructeur lit le fichier pour consruire le vecteur de tous les avions
{
    ...
    for(int i=0;i<nb_avions;i++)
    {
        ...
        m_liste_avions.push_back(new Avion(matricule,type,consomation,capacite,vitesse,0,0,0,img,coo,false,trajet)); //création+ajout au vecteur
    }
}
///GETTERS/SETTERS
...
void AllAvions::affichage()
{
    ...
    for(auto a:vect)
    {
        ...
        std::cout<<"    Capacite reservoir : "<<a->get_capacite()<<" L";
        std::cout<<"     Vitesse  : "<<a->get_vitesse()<<"km/h"<<std::endl;
    }
}
