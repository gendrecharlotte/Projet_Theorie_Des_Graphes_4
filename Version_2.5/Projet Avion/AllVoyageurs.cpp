...
AllVoyageurs::AllVoyageurs(std::string Fichier, AllAvions* Avions)
{
    ...
    int matricule, type, consommation, capacite;
    int horaire;
    ...
    while(myFile.good())
    {
        ...
        BITMAP *img;
        Coordonnees coo(0,0,0);
        std::stack<InfosAeroport*> trajet;

        //ajout dans le vecteur de voyageurs
        m_liste_voyageur.push_back(new Voyageur (nom, depart, arrive, (new Avion (matricule, type, consommation, capacite,0,0,0,0,img,coo,false,trajet)), horaire));
