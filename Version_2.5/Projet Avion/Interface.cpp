...
void Interface::test(AllAvions avions,DifferentsAeroports aeroports,Simulation simulation)
{
    ...
    for(auto a:avions.get_liste_avions())
    {
        ...
        else if(a->get_coo().get_z()>10 && a->get_type()==1)
        {
            a->set_img(load_bitmap("images/avion_11.bmp",NULL));
        }
        else if(a->get_coo().get_z()>10 && a->get_type()==2)
        {
            a->set_img(load_bitmap("images/avion_22.bmp",NULL));
        }
        else if(a->get_coo().get_z()>10 && a->get_type()==3)
        {
    ...
    while(!key[KEY_ESC])
    {
        ...
        for(auto a:avions.get_liste_avions())
        {
            ...
            traj=a->get_trajet();
            draw_sprite(page,a->get_img(),a->get_coo().get_x(),a->get_coo().get_y());
            std::cout<< "y= "<<a->get_coo().get_y()<<"x= "<< a->get_coo().get_x() <<"z= " <<a->get_coo().get_z() << std::endl;
            //sleep(1);
            if(traj.size()!=0)
            {
            ...
            {
                if (a->get_coo().get_y()==k->get_coo().get_y() && a->get_coo().get_x() == k->get_coo().get_x())
                {
                    if(a!=k)
                    {
                            ...
                            a->set_coo(coo);
                            if(a->get_coo().get_z()>10 && a->get_type()==1)
                            {
                                a->set_img(load_bitmap("images/avion_11.bmp",NULL));
                            }
                            else if(a->get_coo().get_z()>10 && a->get_type()==2)
